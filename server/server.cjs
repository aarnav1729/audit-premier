...
// multer for evidence uploads (to disk)
...
app.post('/api/audit-issues', async (req, res) => {
  const {
    fiscalYear, date, process, entityCovered, observation,
    riskLevel, recommendation, managementComment,
    personResponsible, approver, cxoResponsible,
    timeline, currentStatus, riskAnnexure,
    actionRequired, iaComments
  } = req.body;
  ...
  try {
    ...
    reqDb.input('managementComment',sql.NVarChar(sql.MAX), managementComment);
    ...
    reqDb.input('evidenceReceived', sql.NVarChar(sql.MAX), JSON.stringify([]));
    reqDb.input('riskAnnexure',     sql.NVarChar(sql.MAX), riskAnnexure);
    reqDb.input('actionRequired',   sql.NVarChar(sql.MAX), actionRequired);
    reqDb.input('iaComments',       sql.NVarChar(sql.MAX), iaComments);

    const insertResult = await reqDb.query(insertQuery);
    const newIssue = insertResult.recordset[0];

    const detailsHtml = `
      <h2>Audit Issue Details</h2>
      <ul>
        <li><strong>Serial #:</strong> ${newIssue.serialNumber}</li>
        <li><strong>Fiscal Year:</strong> ${newIssue.fiscalYear}</li>
        <li><strong>Date:</strong> ${new Date(newIssue.date).toLocaleDateString()}</li>
        ...
        <li><strong>Management Comment:</strong> ${newIssue.managementComment || '–'}</li>
        <li><strong>Timeline:</strong> ${newIssue.timeline ? new Date(newIssue.timeline).toLocaleDateString() : '–'}</li>
    ...
    await Promise.all([
      ...
      sendEmail(
        ...
        'You have been assigned as the CXO Responsible for a New Audit Issue',
        detailsHtml
      ),
      sendEmail(
        newIssue.approver,
        'You have been added as the Approver for a new Audit Issue',
        detailsHtml
      )
    ]);
    ...
    console.error('⛔ Error creating audit issue & sending emails:', err);
  ...
  try {
    ...
    const issues = result.recordset.map(r => {
      ...
      if (typeof r.evidenceReceived === 'string') {
        try { ev = JSON.parse(r.evidenceReceived); }
        catch { ev = []; }
      }
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (rows.length < 2) {
        return res.status(400).json({ error: 'File must have header and at least one row' });
      }
      ...
      for (const row of dataRows) {
        ...
        try {
          const [
            ,
            fiscalYear, proc, entityCovered, observation, risk,
            recommendation, managementComment, personResponsible, approver,
            cxoResponsible, timeline, currentStatus, , reviewComments,
            riskDup, riskAnnexure, actionRequired, managementComments2,
            iaComments
          ] = row;
          ...
          reqDb.input('cxoResponsible',   sql.VarChar(256), cxoResponsible || '');
          reqDb.input('timeline',         sql.Date, timeline || null);
          reqDb.input('currentStatus',    sql.VarChar(50), mapStatus(currentStatus));
          reqDb.input('evidenceReceived', sql.NVarChar(sql.MAX), JSON.stringify([]));
          reqDb.input('riskAnnexure',     sql.NVarChar(sql.MAX), riskAnnexure || '');
          reqDb.input('actionRequired',   sql.NVarChar(sql.MAX), actionRequired || '');
          reqDb.input('iaComments',       sql.NVarChar(sql.MAX), iaComments || '');
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (row.evidenceReceived) {
        try { currentEvidence = JSON.parse(row.evidenceReceived); }
        catch { currentEvidence = []; }
      }
      ...
      await pool.request()
        .input('id', sql.UniqueIdentifier, issueId)
        .input('evidenceReceived', sql.NVarChar(sql.MAX), JSON.stringify(updatedEvidence))
        .query(`
      ...
      await Promise.all([
        ...
        sendEmail(row.cxoResponsible, `${uploadedBy} submitted proof for ${caption}`, htmlForCXO),
        sendEmail(row.approver, `New Proof Uploaded for ${caption}`, htmlForAuditor)
      ]);
...
app.put('/api/audit-issues/:id/review', async (req, res) => {
  ...
  if (!['Accepted', 'Insufficient'].includes(evidenceStatus)) {
    return res.status(400).json({ error: 'evidenceStatus must be "Accepted" or "Insufficient"' });
  }
  ...
  try {
    ...
    const { serialNumber, process, entityCovered, personResponsible, cxoResponsible } = lookup.recordset[0];
