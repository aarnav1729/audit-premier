  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await Promise.all(formData.entities.map(async ent => {
        ...
        if (formData.coverageStartMonth) payload.append('coverageStartMonth', formData.coverageStartMonth)
        if (formData.coverageEndMonth) payload.append('coverageEndMonth', formData.coverageEndMonth)
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <Label className="font-semibold text-base">Audit Coverage (optional)</Label>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={e => setFormData(fd => ({ ...fd, coverageStartMonth: e.target.value }))}
                />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={e => setFormData(fd => ({ ...fd, coverageEndMonth: e.target.value }))}
                />
              </div>
            </div>
          </div>
                ...
                <div key={index} className="flex items-center gap-2 mt-1">
                  ...
                  {index > 0 && (
                    <button
                      type="button"
                      onClick={() => {
                        const newList = formData.approver.filter((_, i) => i !== index);
                        setFormData(fd => ({ ...fd, approver: newList }));
                      }}
                ...
                <div key={index} className="flex items-center gap-2 mt-1">
                  ...
                  {index > 0 && (
                    <button
                      type="button"
                      onClick={() => {
                        const newList = formData.cxoResponsible.filter((_, i) => i !== index);
                        setFormData(fd => ({ ...fd, cxoResponsible: newList }));
                      }}
                      className="text-red-500 hover:underline"
                    >
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                onChange={e => setFormData(fd => ({ ...fd, managementComment: e.target.value }))}
              />
            </div>
